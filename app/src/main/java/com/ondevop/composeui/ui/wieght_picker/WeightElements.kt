    ...
    Canvas(modifier = modifier
            ...
            detectDragGestures(
                onDragStart = {offset->
                    dragStartedAngle = atan2(
                        circleCentre.x - offset.x,
                        circleCentre.y-offset.y
                    ) * (180 / PI.toFloat())
                ...
                val touchAngle = -atan2(
                    circleCentre.x - change.position.x,
                    circleCentre.y-change.position.y
                ) * (180 / PI.toFloat())
        ...
        for(i in minWeight..maxWeight){
            ...
            val lineStart = Offset(
                x = (outerRadius - lineLength ) * cos(angleInRad) + circleCentre.x,
                y = (outerRadius -lineLength ) * sin(angleInRad) + circleCentre.y
            )

            val lineEnd = Offset(
                x = (outerRadius  ) * cos(angleInRad) + circleCentre.x,
                y = (outerRadius ) * sin(angleInRad) + circleCentre.y
            )

            drawContext.canvas.nativeCanvas.apply {
            if (lineType == LineType.TenStep){
                val textRadius = outerRadius -lineLength - 5.dp.toPx() - style.textSize.toPx()
                val x = textRadius * cos(angleInRad) + circleCentre.x
                val y = textRadius * sin(angleInRad) + circleCentre.y
            ...
            val middleTop = Offset(
                x = circleCentre.x,
                y = circleCentre.y - innerRadius - style.scaleIndicatorLength.toPx()
            )

            val bottomLeft = Offset(
                x = circleCentre.x - 4f,
                y = circleCentre.y - innerRadius
            )
